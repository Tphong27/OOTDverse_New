...
export default function OrderCard({ order, role, onViewDetail }) {
  ...
  // Get partner info (buyer or seller)
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {partner && (
                <div className="flex items-center gap-2 mt-2 text-sm text-gray-600">
                  ...
                  />
                  <span>
                    {role === "buyer" ? "Người bán" : "Người mua"}: {partner.fullName}
                  </span>
                </div>
              )}
            </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 lg:gap-6">
            ...
            <div className="flex gap-2">
              ...
              {order.order_status === "delivered" && role === "buyer" && !order.buyer_rating && (
                <button className="px-4 py-2 rounded-lg bg-yellow-500 text-white font-medium hover:bg-yellow-600 transition-colors">
                  Đánh giá
                </button>
              )}

              {order.order_status === "pending_payment" && role === "buyer" && (
                <button className="px-4 py-2 rounded-lg bg-pink-500 text-white font-medium hover:bg-pink-600 transition-colors">
                  Thanh toán
                </button>
              )}
            </div>
