// frontend/src/components/marketplace/MyListingsTab.jsx
...
export default function MyListingsTab() {
  const { user } = useAuth();
  const { myListings, loading, loadMyListings, removeListing, boostListing } =
    useMarketplace();
  ...
  useEffect(() => {
    if (user) loadMyListings(filters);
  }, [user, filters]);
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
            ...
            <p className="text-gray-600 mb-6">
              Bạn có chắc chắn muốn xóa món đồ "
              {deleteConfirm.item_id?.item_name}"? Hành động này không thể hoàn
              tác.
            </p>
            <div className="flex gap-3">
              <button
